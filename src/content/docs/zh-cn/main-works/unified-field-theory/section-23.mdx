...
但在 $r$ 趋近于零【也可以说空间点 $p$ 无限趋近于 $o$ 点】，且 $o$ 点可以看成一个无限小的球体的情况下，式子出现了 $\frac{0/0}$ 的情况，利用狄拉克 $\delta$ 函数，可以得到：
...
$$
\begin{align}
A_x &= - G\ m\ \gamma\ \dfrac{x - vt}{[\gamma^2(x-vt)^2+y^2+z^2]^{\dfrac{3}{2}}} \nonumber \\
A_y &= - G\ m\ \gamma\ \dfrac{y}{[\gamma^2(x-vt)^2+y^2+z^2]^{\dfrac{3}{2}}} \nonumber \\
A_z &= - G\ m\ \gamma\ \dfrac{z}{[\gamma^2(x-vt)^2+y^2+z^2]^{\dfrac{3}{2}}} \nonumber
\end{align}
$$
...
$$
\begin{equation}\nonumber
\vec{A} = - G\ m\ \gamma\ \dfrac{(x - vt)\vec{i} + y\vec{j} + z\vec{k}}{[\gamma^2(x-vt)^2+y^2+z^2]^{\dfrac{3}{2}}}
\end{equation}
$$
...
$$
\begin{equation}\nonumber
\vec{A} = - \dfrac{G\ m}{\gamma^2\ r^2(1 - \beta^2\ sin^2\theta)^{\dfrac{3}{2}}}\ \vec{e_r}
\end{equation}
$$
